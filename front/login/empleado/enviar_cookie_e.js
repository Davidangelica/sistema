function obtenerValorDeCookie(nombre) {
  ...
  function enviarCookie(cookieValue) {
    ...
    const url = `http://127.0.0.1:8000/menuPrincipal/empleado/${encodeURIComponent(cookieValue)}`;
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    botonInicio.addEventListener("click", async function() {
        ...
        if (valorCookie !== null) {
            ...
            window.location.href = `http://127.0.0.1:8000/menuPrincipal/empleado/${encodeURIComponent(valorCookie)}`
        }
    });
  });
